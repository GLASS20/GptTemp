...
class InvManager : Module("InvManager","Auto clean your inv",ModuleCategory.Util, Keyboard.KEY_Z) {
...
//    private val ignoreDurabilityUnder = FloatValue("IgnoreDurabilityUnder", 0.3f, 0f, 1f)
    ...
    private val sortSlot4Value = ListValue("SortSlot-4", items, "Potion").displayable { sortValue.get() }
    private val sortSlot5Value = ListValue("SortSlot-5", items, "None").displayable { sortValue.get() }
    private val sortSlot6Value = ListValue("SortSlot-6", items, "None").displayable { sortValue.get() }
    private val sortSlot7Value = ListValue("SortSlot-7", items, "None").displayable { sortValue.get() }
    private val sortSlot8Value = ListValue("SortSlot-8", items, "Pearl").displayable { sortValue.get() }
    private val sortSlot9Value = ListValue("SortSlot-9", items, "Block").displayable { sortValue.get() }
    ...
    fun onUpdate(event: EventUpdate) {
        if (noMoveValue.get() && PlayerUtils.isMoving() ||
		...
		val currItem = stack.item
                ...
                items(0, 45).none { (_, stack) ->
                    if (stack != itemStack && stack.javaClass == itemStack.javaClass) {
                        ...
                        if (damage == dmg) {
                            val currDamage = item.maxDamage // getDamage(itemStack)
                            currDamage >= stack.item.maxDamage //.getDamage(stack)
                        } else damage < dmg
                    } else {false}
                }
                ...
                items().none { (_, stack) ->
                    if (itemStack != stack && stack.item is ItemBow) {
                        ...
                        if (currPower == power) {
                            val currDamage = item.maxDamage(itemStack)
        ...
        val armorPieces = IntStream.range(0, 36)
            .filter { i: Int ->
                ...
                (itemStack != null && itemStack.item is ItemArmor &&
                        (i < 9 || System.currentTimeMillis() - (itemStack as ItemStack).itemDelay >= itemDelayValue.get()))
            }
    ...
    private fun items(start: Int = 0, end: Int = 45): Map<Int, ItemStack> {
        ...
        for (i in end - 1 downTo start) {
            ...
            if (System.currentTimeMillis() - (itemStack as ItemStack).itemDelay >= itemDelayValue.get()) {
                items[i] = itemStack
            }
        }
